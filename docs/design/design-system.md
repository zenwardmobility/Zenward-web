...
No production photography exists yet. `src/components/layout/PhotoPlaceholder.tsx` renders a deliberate, honestly-labeled gradient placeholder — this project does not generate or use AI photorealistic imagery as a stand-in for real brand photography (see [reference-index.md](./reference-index.md)). Replace `PhotoPlaceholder` usages with real `next/image` photography once approved images exist.
